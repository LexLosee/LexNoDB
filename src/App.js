    ...
    const list = this.state.products.map((product, i) => {
      return (
        <div key={i} className="list_product">
          ...
          <img className="product_img" src={product.img_url} alt="product img"></img>
          <button className="delete_btn" onClick={() => this.handleDelete(product.id)}>Delete<img className="trash_icon" src={trash} alt="product img"></img></button>
    ...
    return (
      <div className="App">
        <div className="header">
          <p class="shadow text1">Roller Derby Wishlist</p>
        </div>
